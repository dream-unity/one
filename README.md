...
The original artwork gently animates without changing its appearance on startup. All ink detail, fine construction lines, connecting spines and paper remain part of the same drawing. Small, slow, coordinated angular movements stay inside the ring bands; the silhouette, labels and three portals keep a fixed size and position. The symbol runs entirely on autopilot: dragging, manual rotation and scene zoom are disabled. There is no swelling, rocking, movement toward and away from the viewer, or control strip.

The `symbol-3d.js` module uses the locally vendored Three.js renderer and the shared rhythm in `symbol-life.js`. A single unlit surface samples the complete, unmodified source image with bounded angular UV flow. The first frame has zero displacement and matches the static image's colour, extent and layout; no reconstructed bronze rings or replacement spine are added. The original accessible HTML portals stay in place throughout loading, animation and fallback. Motion starts paused for reduced-motion preferences, and rendering stops behind a world dialog or hidden tab. Unsupported WebGL or a lost graphics context exposes the same functional original illustration.
