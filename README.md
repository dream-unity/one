...
The original artwork forms one continuous living 3D surface. Its ink, construction guides, connecting spine and parchment retain the original colours and layout. A shared slow rhythm circulates the ring details and flexes their raised contours, with balanced left/right motion and soft cast shadows. The body, camera, labels and three portal positions remain anchored; there is no zoom, movement of the body toward and away from the viewer, manual dragging, or control strip.

The `symbol-3d.js` module uses the locally vendored Three.js renderer and the 36-second rhythm in `symbol-surface.js`. One connected mesh carries the original RGB artwork with its UVs attached to its vertices. Four smoothly blended fields provide bounded tangential motion and raised, flexing relief; overlapping fields are normalized, and the spine, labels and outer guides are anchored. This preserves eccentric and connected source strokes instead of cutting them into separately tilted circular slices. Surface normals and the shadow pass use the same deformation as the visible mesh. The first frame aligns with the source, then eases into spatial motion; original HTML portals never move. Motion starts paused for reduced-motion preferences, and rendering stops behind a world dialog or hidden tab. Unsupported WebGL or a lost graphics context exposes the functional original illustration.
