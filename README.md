...
The original artwork forms a living 3D mechanism with nine slowly rotating ring layers. Its ink, construction guides, connecting spine and parchment retain the original colours and layout. Individual rings visibly incline and turn through depth, with matte sides and soft cast shadows. The body, camera, labels and three portal positions remain anchored; there is no zoom, movement of the body toward and away from the viewer, manual dragging, or control strip.

The `symbol-3d.js` module uses the locally vendored Three.js renderer and the shared rhythm in `symbol-life.js`. Nine depth-tested annular meshes sample the original RGB artwork, with bevelled faces, extruded side walls, directional shading and real shadow mapping. Their X/Y inclinations are deliberately visible: do not cancel their projection or replace them with a flat UV animation. The stationary connector is separated from rotating surfaces using adjacent ink at the same radius, and the original spine stays above them. Flat ring copies are removed from the backing so inclined surfaces reveal parchment without a second drawing underneath; sparse outer guides remain on the paper. The first frame aligns with the source, then eases into spatial motion; original HTML portals never move. Motion starts paused for reduced-motion preferences, and rendering stops behind a world dialog or hidden tab. Unsupported WebGL or a lost graphics context exposes the functional original illustration.
