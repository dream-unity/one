...
The original artwork forms one continuous living 3D surface. Its ink, construction guides, connecting spine and parchment retain the original colours and layout. A continuous travelling wave moves through shallow 3D relief, with mirrored left/right motion and soft cast shadows. Every ink stroke retains its original position and proportions. The body, camera, labels and three portal positions remain anchored; there is no zoom, movement of the body toward and away from the viewer, manual dragging, or control strip.

The `symbol-3d.js` module uses the locally vendored Three.js renderer and an 18-second rhythm in `symbol-surface.js`. The source X/Y coordinates and UVs stay fixed on one connected mesh; only genuine Z relief and its normals change. This keeps all original circles, joins and proportions intact throughout the animation. The travelling wave circulates continuously instead of slowly twisting the drawing back and forth. Height and slope coefficients are computed once, shared by the surface and shadow pass; a smaller mesh and a display-sized drawing buffer reduce rendering work. Active elapsed time advances independently of frame rate, with suspension time excluded. The complete source square fits the viewport without landscape enlargement or cropping. The first frame aligns with the source, then eases into spatial motion; original HTML portals never move. Motion starts paused for reduced-motion preferences, and rendering stops behind a world dialog or hidden tab. Unsupported WebGL or a lost graphics context exposes the functional original illustration.
