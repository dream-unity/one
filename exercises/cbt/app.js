...
} from './engine.js?v=cbt-clear-20260907';
import {TRANSFER_CASES} from './scenarios.js?v=cbt-clear-20260907';
...
function learningGuide(){
 return `<details class="learning-guide"><summary>How this works, step by step</summary><div class="guide-copy"><p>You can begin with a picture and a few choices. The same steps also let you test hard questions: what caused a problem, what a clue can prove, and when to change your mind.</p><dl>
 <dt>Keep what happened apart from what you think it means.</dt><dd>“Two parts need changing” is something the note says. “Nobody wants me here” is a thought about the note. A thought may be right, wrong, or still untested. The feeling it brings is real even when the thought needs checking.</dd>
 <dt>Look at how a thought can keep going.</dt><dd>Ari feels upset and hides a picture. Then Ari misses a chance to get help. The worry has less chance to change. This is a <strong>maintenance loop</strong>: one step helps the next step keep happening. Feelings, thoughts and actions can affect each other in more than one direction. This story shows one possible loop.</dd>
 <dt>Hold more than one possible reason.</dt><dd>A dot might be missing. A rule might have changed. A counting step might need practice. Two reasons can fit at once. A <strong>working explanation</strong>, sometimes called a model, is an idea you can check and change.</dd>
 <dt>Choose what you expect before you look.</dt><dd>Ask, “If my idea is right, what should I find?” That is a <strong>prediction</strong>. Say what could change your mind too. Keeping the first guess lets you compare it with the result, instead of quietly changing what you meant after seeing the answer.</dd>
 <dt>Choose a check that can answer the question.</dt><dd>A kind reply may help Ari feel better without showing why the picture was marked. A rule card or a clear question may show more. Asking a question can also change the reply you get. A <strong>behavioural experiment</strong> means trying a chosen action and checking what happens.</dd>
 <dt>Check whether two clues came from one place.</dt><dd>If Sam repeats Jo’s note, there is still only one first check. If both people check for themselves, their matching results can tell you more. This is about <strong>source independence</strong>: did each clue come from a separate check? Separate checks can still share a mistake, such as using the same wrong rule.</dd>
 <dt>Change just what the clues ask you to change.</dt><dd>A real mistake still needs care. A changed thought does not erase it. A missing answer stays missing. This is <strong>updating</strong>: make your next thought fit what you learned, including the parts you still do not know.</dd>
 <dt>Take a useful step, then know when to return.</dt><dd>Fix the part you can fix. Ask for a rule you still need. Choose a time to check again. A new clue can be a reason to return sooner. Reading the same note again does not make it a new clue.</dd>
 <dt>Try the idea in a different story.</dt><dd>Remembering the steps and choosing well in a new situation are different tasks. The new stories ask you to choose both an action and a reason. This is <strong>transfer</strong>: using something you learned somewhere new. Success here does not prove that it will work in every part of life.</dd>
 </dl><p>The 12-minute practice can add two closer comparisons: shared clues, and two questions asked from the same starting point. Those replies are made-up examples. They show what to look for; they cannot tell us how every real person will reply.</p></div></details>`;
}
function about(){
 ...
 `<p>A thought can feel true before we know if it is true. We can look for clues, try a small step, and think again.</p><p>Sometimes a worry is right. Sometimes we do not know yet. This practice makes room for both.</p><p>These are made-up stories. You do not have to tell us about your life.</p>${learningGuide()}<details class="learning-guide"><summary>What has been tested?</summary><div class="guide-copy"><p>CBT is used in structured therapy. This exercise practises some of its skills. It is not a full course of therapy, and this exercise itself has not been tested as a treatment.</p><p>The four-minute practice covers a guess, a check, a changed thought and a next step. Eight minutes adds a fuller story and a new case. Twelve minutes adds closer comparisons and another case when time allows. These times are choices for practice, not tested treatment doses.</p><p>Practice uses 24 new stories across 12 kinds of problem. It can bring back a skill you found hard in a different story. After two successful tries on your own in different stories, some instructions can become shorter. Help stays available. These rules do not prove that you have mastered a skill.</p><p>After at least a day, saved practice can bring back an earlier skill in a fresh story. Remembering a step, using help and choosing on your own are counted separately.</p><p><a href="https://www.nice.org.uk/guidance/ng222/chapter/recommendations" target="_blank" rel="noopener noreferrer">Read the CBT guidance</a> · <a href="https://pubmed.ncbi.nlm.nih.gov/21819813/" target="_blank" rel="noopener noreferrer">Read a study of thought records and experiments</a></p></div></details><details class="learning-guide"><summary>What is saved?</summary><div class="guide-copy"><p>Practice stays on this page unless you choose to save it on this device. Saved practice keeps the story, skill, result, time and whether you used help. It does not keep a personal story or recording. You can clear it from the start screen.</p></div></details>`);
}
function intro(){
 session=null;running=false;$('#session-controls').hidden=true;
 app.innerHTML=`<section class="intro" id="stage" tabindex="-1"><p class="eyebrow">Dream Machine / Mind / Model</p><h1>Check a thought.</h1><p class="method-name">Cognitive behavioural therapy · CBT</p><p class="intro-copy">A thought is an idea in your mind. It can change how you feel and what you do. Look for clues. Try a small step. See what you learn.</p><div class="method-preview" aria-label="Look, try, learn"><span><i aria-hidden="true">◉</i>Look</span><b aria-hidden="true">→</b><span><i aria-hidden="true">◇</i>Try</span><b aria-hidden="true">→</b><span><i aria-hidden="true">✧</i>Learn</span></div><p>Use a made-up story. You do not need to write or tell us about yourself.</p><p class="field-label" id="duration-label">How long would you like?</p><div class="duration-group" role="group" aria-labelledby="duration-label">${[[4,'A short try'],[8,'A full try'],[12,'More to work out']].map(([n,label])=>`<button type="button" class="duration-option ${minutes===n?'selected':''}" aria-pressed="${minutes===n}" data-action="duration" data-value="${n}"><strong>${n} min</strong><span>${label}</span></button>`).join('')}</div><label class="preference"><input id="self-paced" type="checkbox" ${selfPaced?'checked':''}> No timer</label><label class="preference"><input id="remember" type="checkbox" ${remember?'checked':''}> Save my practice on this device</label><p class="status-note">${remember?'You can clear saved practice at any time.':'Nothing is saved after you close this page.'}</p>${progress.sessions?`<p class="status-note">You have practised ${progress.sessions} ${progress.sessions===1?'time':'times'}. <button class="quiet" data-action="clear-progress">Clear saved practice</button></p>`:''}<div class="action-bar">${primary('Start',false,'start')}<span class="timer-note">You can pause, stop, or take more time.</span></div>${learningGuide()}</section>`;
}
function begin(){
